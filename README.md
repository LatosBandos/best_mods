...
Antique Atlas 4
> Добавляет в игру атлас, который служит глобальной картой в стиле нарисованной своими руками, все функции от других модов на карту присутствуют, кроме просмотра мобов на карте
...
Armor Set Bonuses (Gameplay)
> Позволяет настраивать доолнительные эффекты или аттрибуты за полный комплект оределенной брони, при установке уже встроены эффекты для стандартной брони, так же настроить эффекты можно на любую комбинацию брони, не только на полный сет
...
Aquatica_ (Content)
> Добавляет новое измерение глубокого океана, в нем представлены 5 биомов различной освещенности, так же новые существа от безобидных до левиафанов
...
Backpacked (Gameplay)
> Один из самых сбалансированных модов на рюкзаки, так как на всю игру делается лишь 1 рюкзак, а его слоты открываются по мере прогрессии, за уровни игрока, так же у рюкзака есть система пассивных улучшений, которые даже не нужно крафтить, но для них за опыт нужно так же открывать ячейку
...
Better HP (Visual)
> Делает очень прикольный интерфейс для основных характеристик игрока, мод поддерживает увеличение максимального предела любого аттрибута
...
Blooming Biosphere (Worldgen)
> Добавляет во все стандартные биомы игры небольшие детали, например упавшие деревья, дождливые грязные места, скалистые образования и новые виды деревьев

Block Pack (Content)
> Добавляет больше вариаций для имеющихся в игре блоков относительно строительства, а так же некоторую мебель и все для обустройства дома внутри и снаружи
...
Celestial Overhaul (Gameplay)
> Меняет все основные механики аттрибутов так, чтобы они лучше вписывались в модовый геймплей, например острота будет давать процентный урон вместо фиксированного увеличение в 0.5 за уровень, а защита перерабатывается так, что каждое увеличение очков защиты дает равномерную прибавку к уменьшению урона, а в оригинале майна при 20 очках защиты ты уже упираешься в потолок из 80% защиты, тут же при 20 очках получается всего 55% защиты

> Я думаю что этот мод будет полезен во всех сборках
...
Client Tweaks (QoL)
> Позволяет включить множество полезных улучшений, например факел не будет ставиться если в левой руке и ты строишь блоки или скрытие собственных частиц от зелий
...
Countered's Terrain Slabs (Worldgen)
> Теперь при генерации мира разные слои земли будут сглаживаться путем добавения полублоков между ними, это работает и под землей

Countered's Smooth Steps (Visual)
> При подьеме вверх по полублокам или ступеням сглаживает резкий подъем камеры, делая геймплей намного чище

Cult of Azazel (Content)
> Добавляет в игру несколько структур и данжей в аду, которые наполнены сектантами и демонами культа. Боссы дропают уникальный лут в виде брони и оружия, так же их не получится так просто победить из-за механики схожей с драконом края, когда нужно сломать все кристалы, чтобы босс не хилился. Для нахождения и призыва главного босса надо найти его маску и компас, в общем неплохой мод для любой сборки
...
Custom Glow Enchantment Outline (Visual)
> Вместо переливания зачарованных вещей теперь вокруг них будет светящаяся обводка, этот мод хорош тем, что работает с любыми модовыми предметами, включая 3д версии предметов
...
Dark Waters (Content)
> Добавляет страшных морских уебищ, я так понял они спаунятся только во время дождя или грозы в мире
...
Difficult Caves (Gameplay)
> Делает так, что мобы постепенно бафаются в зависимости от глубины в пещерах, на поверхности у зомби всего лишь есть шанс получить сопротивление к откидыванию, а глубоко под землей мобы уже имеют фул сеты разной низкоуровневой брони и такое же оружие
...
Epic Fight (Gameplay)
> Меняет боевку игры, добавляя разновидности атакам игрока, по типу атаки в прыжке, с разбега или специальной атаки, есть множество дополнений
...
Enchanted Books: Re-covered (Resourcepack)
> Изменяет внешний вид книг зачарования в зависимости от наложенного зачарования, отличается от мода тем, что тут иконки книг более визуально понятные
...
Extra Compat (Optimization)
> Добавляет много совместимостей для самых известных модов, чтобы исправить их работоспособность, возможно увеличить фпс и предотвратить некоторые краши
...
FastFurnace (Optimization)
> Оптимизирует число вызовов сервера при обработке рецепта в печи, что значительно снижает нагрузку на ядро

FastWorkbench (Optimization)
> Оптимизирует число вызовов сервера при обработке рецепта в верстаке, что значительно снижает нагрузку на ядро
...
Field Guide (Gameplay, QoL)
> Теперь с помощью подзорной трубы можно сканировать различных мобов, постройки и даже растения, получая о них общую информацию, неплохая замена модам на отображение хп мобов и всякой доп информации, которая изначально не должна быть известна игроку
...
Foliage+ (Resourcepack)
> Делает листву и траву более пышной, придавая всей растительности реализма
...
Geolosys (Worldgen)
> Меняет генерацию руд в мире, теперь на поверхности будут появляться небольшие кусочки руды, под которыми глубоко находится огромная залежь этой самой руды
...
Grand Teleport (Visual)
> Добавляет крутую анимацию при телепортации игрока в другое место, как будто он переключается между персонажами в GTA V, этот эффект так же работает при телепортации между Waystones
...
Grim Kingdoms: Lost structures & ruins (Worldgen)
> По миру будут спауниться средние и огромные города и крепости, все они будут либо заброшены, либо походить на руины
...
Immersive Enchanting (Gameplay)
> Теперь зачарование позволяет выбирать любое нужное игроку зачарование, но для этого нужно сделать специальные полки и заполнить их книгами, чтобы видеть зачарования, так же зачарования нельзя сразу сделать максимального уровня, а для каждого уровня нужен лазурит + материал соответствующий зачарованию
...
Immersive Gateways (Gameplay, Worldgen)
> По миру будут появляться порталлы, которые переносят тебя к случайному такому же порталлу где-то в другой части мира, помогает быстро исследовать новые места и биомы
...
Infinity Cave (Worldgen)
> Теперь на высоте с -64 до -128 будет располагаться невероятно огромная пещера, которая имеет свои 5 биомов, а так же свои структуры
...
Iter RPG (Content)
> Добавляет в игру много структур в стиле рпг, так же новых боссов и предметы (автор долбаеб не написал нихуя)

Ixeris (Optimization)
> Стабилизирует фпс при резком повороте мышью, особенно при высокой частоте опроса самой мыши (выше 1000Hz)
...
Litosphere (Worldgen)
> Изменяет генерацию мира, делая ее более реалистичной, теперь около океанов будут либо большие пляжи, либо скалы, реки не будут перекрываться странной генерацией и будут идти далеко вперед, теперь везде могут сгенерироваться озера и еще куча изменений
...
Majrusz's Accessories (Content)
> Добавляет некоторые аксессуары для увеличения характеристик, для разнообразия можно поставить в некоторые сборки

Man of many planes (Content)
> Самолеты (круто)

Matcha Flavoured (Datapack)
> Очень обширный датапак, который перерабатывает почти каждый аспект игры, например переработка торгов с жителями, зачарований, полностью новые виды брони, альтернативная ранняя прогрессия, другая система функциональности еды и так далее

MC Dungeons Enchanting (Content)
> Меняет систему зачарований на систему из minecraft:dungeons

Mebahel's Creatures (Content)
> Добавляет в игру огромную структуру в виде подземелья в стиле стимпанк, населенное такими же механическими мобами и боссами
...
Modifiers (Gameplay)
> Теперь у предметов в игре будут модификаторы схожие с теми, что есть в террарии, например на мече +15% к урону и +9% к скорости и так далее, менять модификаторы можно на кузнечном столе с помощью слитка, который чинит текущий предмет
...
Mining Quakes (Visual)
> Добавляет анимацию дрожания блока при его ломании, все можно настроить

Myths & Legends (Content)
> Добавляет в игру около десятка мобов в стиле темного фентези, немного предметов, включая аксессуары, а так же парочку структур в виде средневековья
...
Player Revive (Gameplay)
> После смерти товарища есть 60 секунд на его возрождение
...
Puddles & Floods (Visual)
> Теперь после дождя будут появляться лужи, реки будут визуально разливаться. Так же лужи будут оставаться от разбивающихся зелий и под капельниками, с которых капает вода.

Punchy! (Visual)
> Продвинутая замена моду Hold My Items, добавляет крутые анимации рук персонажа от первого лица, детализация доходит до такого, что если игрок копает грязь, то его руки становятся такими же грязными и потом если встать в воду, то игрок начнет мыть руки. При этом мод очень хорошо совместим с кастомными оружиями из других модов, в общем ставить следует во все сборки
...
Random Crits (Gameplay)
> Название говорит само за себя, перерабатывает систему критического удара, теперь крит не будет если атаковать с прыжка, теперь шанс на крит всегда 12%, но его можно увеличить эффектом удачи на 6% или если у персонажа будет менее 3.5 хп, то есть шанс на эффект адреналина, который дает еще +20% шанса крита

Re:Deco (Visual)
> Добавляет очень много новых блоков для обустройства дома, например камодов или баночек с растениями

Rainbow's foliage: vanilla (Resourcepack)
> Один из лучших текстурпаков, который изменяет вид листьев деревьев, делая их более пышными и детализированными не просто добавляя доп текстуры к уже имеющемуся блоку листвы, а перерабатывающий его вид впринципе
...
Refined Torches (Texture Pack)
> Теперь факела будут иметь нормальную анимацию огня как в инвентаре, так и в руках, этот текстур пак полностью совместим с Punchy!

Refined Buckets (Texture Pack)
> Ведра с водой будут в 3д, а так же вода в них будет пропадать если ты погрузишься под воду и возвращаться при всплытии. Если в ведре будет моб по типу аксолотля, то при погружении в воду с этим ведром он будет плавать около тебя. Текстур пак полностью совместим с Punchy!
...
RUTHLESS (Gameplay)
> Враждебные мобы становятся намного более опасными за счет добавления им новых механик, например теперь крипер может ускоряться чтобы быстрее дойти до игрока, зомби могут ломать блоки и подбирать ближайшее оружие, так же пытаются сесть на паука чтобы стать более живучими, эндермен может телепортировать игрока в воздух, а так же многие мобы будут преследовать игрока до самой смерти
...
Shadow Drop (Visual)
> Добавляет всем предметам игры тень при просмотре в инвентаре, работает абсолютно со всеми модами, так как для теней берет саму текстуру каждого предмета

Shield Overhaul (Gameplay)
> Теперь при нажатии лкм при зажатом щите игрок делает рывок и станит того, кто стоял перед ним, так же теперь есть несколько фреймов для парирования, при попадании в него щит тоже застанит врага
...
Still Life (Worldgen)
> Дополнение для мода Litosphere, добавляет некоторые переходные биомы и дополняет так же пещерные биомы

> *примечание*: Litosphere нужно установить в качестве мода и Still Life должен быть выше в папке модов

Structory (Worldgen)
> Добавляет структуры, которые не особо содержат лута, нужны просто для атмосферности в мире, большинство из них заброшены или вообще являются руинами

Skarrier Mobs (Content)
> Добавляет множество новых мобов, в основном враждебных и выглядящих как искаженные и более страшные версии того, что уже есть в игре. Так же добавляет новый сет брони и инструментов.

> *примечание:* Мод все еще в альфа версии, так что контент в нем не особо полноценен, но играбелен
...
Sizeable Foliage (Visual)
> Разнообразит размер текущей растительности, чтобы она не так часто повторялась, например теперь некоторые кусты могут быть больше игрока или трава быть в 2 раза меньше обычного размера, эти изменения применяются почти ко всем видам растительности

## (T)

Taller Trees (Worldgen)
> Делает все деревья в игре выше на 2 блока, что делает перемещение по лесу более простым, не ломая лишнюю листву, а так же леса станут казаться больше
...
Too Many Bows (Gameplay, Content)
> Добавляет около десятка новых луков с особыми способностями, полностью совместим с better combat, так же добавляет новые зачарования и аксессуары специально для лучника
...
Untamed Wilds (Content)
> Разнообразит игру путем добавления различных животных из реального мира и так же новых вариаций уже существующих растений игры, неплохо дополняет мир в целом

Unique Accessories (Gameplay)
> Очередной мод на аксессуары, только способы их получения различаются от простого лутания сундуков, можно купить их у жителей или найти в подозрительном песке, для разнообразия тоже пойдет

## (V)

Vanilla Backport (Content)
> Добавляет некоторый контент из новых версий игры в старые, чтобы пользоваться самыми новыми функциями на любой из версий

Village Spawn Point (QoL)
> Теперь при создании нового мира игрок всегда будет появляться в деревне

Visual Health (Visual, QoL)
> При опускании здоровья врага до определенного процента, на нем будут видны визуальные повреждения, на игроке это так же будет заметно. Текстуры повреждений меняются в зависимости от того, кто и чем наносил урон. Мод должен работать почти со всеми другими модами на мобов.
...
Wallpapers (Visual)
> Позволяет наносить на разные стороны блоков обои различных цветов и узоров, так же помимо обоев есть и другие мелкие детали для стен, единственный минус, что обои нельзя клеить на полублоки и ступеньки
...
Wild Vanilla - Better plants and flowers (Resourcepack)
> Изменяет стандартную траву и все растения игры либо на продвинутые 2д версии, либо на полноценные 3д версии, с ощущением того, что мы находимся во временах динозавров, но для обычной игры тоже вполне подходит, так же совместим с другими текстурпаками изменяющими траву

Wthit (QoL)
> Показывает название блока и из какого он мода

Wild Bushes (Worldgen)
> Добавляет в ландшафры кусты случайных видов растений, иногда даже с полезными культурами по типу моркови
...
Yuushya Townscape (Content, Visual)
> Огромный мод на новые блоки и декорации в стиле городской и деревенской тематики, добавляет около 2600 новых блоков, позволяет невероятно детализировать свои постройки 
